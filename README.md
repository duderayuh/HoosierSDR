...
- **Places and runs** — a book of hospitals, stations and landmarks, with what each can do and the channels that belong to it; it can fill itself in from the hospital channels your catalog already names. A tripwire can watch the dispatch map instead of the radio: a kind of run, near a place or near the nearest one with a cath lab, and — if you want to wait for it — where the patient actually went, because the crew's hospital report is joined back to the run that started it by the callsign they say.
- **Addresses that would not geocode** — the grid reference the dispatcher reads out is learned from the calls that did place, and then used both to pin a call and to tell a mis-heard street name from the real one. On one real library it took the unplaced share from 30 % to 6 %. Distances can come from a router running on your own machine (OSRM in a container, one state's map) instead of a straight line.
