...
## Places, addresses, incidents (2026-09-12)

`app/src/addr.rs`, `app/src/places.rs`, `app/src/link.rs`, `app/src/routing.rs`.

**Addresses the geocoder refuses.** Dispatchers read a grid reference after the address ("… 1834 hours, location 7700 South, 1100 West"). `addr.rs` fits that grid from calls that *did* geocode — Theil–Sen over every pair, so a mis-heard digit does not drag it — and then places a call from the reference alone, or uses it as a referee: a street name that sounds like one the geocoder has confirmed before (`fuzzy.rs`) and lands near where the grid says is a correction, geocoded properly. It never swaps a number or flips a direction word without the grid insisting. Pins say how they got there (`geocode` = `grid` | `corrected`), and a re-geocode upgrades an approximate pin when the address later resolves. On a real library, unplaced dispatches went 145 → 30, ten of them to exact addresses.

**Places.** `places.json`: hospitals, stations and landmarks with a point, features (a cath lab, a stroke team — never guessed, always ticked by the listener), the talkgroups that belong to them and the system those talkgroups are on. "Add from my catalog" reads hospital names out of the listener's own RadioReference descriptions ("Med 03 - … ER"), skipping spares.

**Incidents joined to hospital reports.** `link.rs` matches a stored conversation to a dispatch by the callsign the crew says — the words a county puts in front of a unit number are learned from its own dispatches — inside a drive-and-report window. One run with that crew is a link; several, and the clock decides; a genuine tie goes to the local model, and an unreachable model means no link. Conversations gain `incident` and `link_how` (added by `ALTER TABLE`, idempotent). Incident details open with the run as a story: dispatched → where the patient went, with the reason the two were joined.

**Incident tripwires.** `when.kind = "incident"` matches on call type, distance from a place or from the nearest place that can do a thing, and whether the crew has reported yet (`linked_only`). Tokens: `{calltype} {address} {units} {summary} {place} {nearest} {km} {mins} {hospital} {report}`. A run is told about once per rule, remembered in `tripwire_events.incident_id`.

**Distances.** `routing.rs` asks a local OSRM (`/route/v1/driving`), falling back to the straight line and saying which. The map is prepared from the app through Docker (extract → partition → customize → routed); the region is read from addresses the listener's geocoder already confirmed. The Docker pipeline is covered by a test against a stand-in binary; the real run is unexercised.

**`HS_NO_SEND=1`** blocks every Telegram send at `alerts::telegram_api` while leaving reads and the rest of the app alone — for running a second copy against a copy of a library.
