...
fn templates() -> Vec<AnalyzerRule> {
    vec![
        ...
        AnalyzerRule {
            ...
            instructions: "You are monitoring a fire/EMS radio talkgroup for cardiac-arrest status. Read this transmission and determine whether chest compressions (CPR) are CURRENTLY in progress — manual or mechanical (e.g. LUCAS) compressions being performed right now, patient still in arrest (pulseless).\n\nCPR IS in progress: 'CPR in progress', 'compressions ongoing/continuing', 'working the code', 'still pulseless', 'shock delivered and compressions resumed', 'LUCAS applied', 'transporting with CPR in progress'.\n\nCPR is NOT in progress when: ROSC ('pulse back', 'ROSC', 'rhythm with pulse', 'now breathing'), pronounced ('time of death', 'pronounced', 'code terminated', 'resuscitation discontinued', 'per medical control'), CPR not yet started (initial dispatch, en route, 'on arrival'), or not a cardiac arrest.\n\nThis is ONE transmission. Answer only what it states. If CPR status is not clearly stated, set cpr_in_progress=false and status='unclear' — do not infer ongoing CPR from a past-tense or ambiguous mention.".into(),
            fields: vec![
                Field { key: "arrest".into(), kind: "bool".into(), desc: "true if this transmission concerns a cardiac arrest / working code".into() },
                Field { key: "cpr_in_progress".into(), kind: "bool".into(), desc: "true if compressions are actively in progress at this moment".into() },
                Field { key: "rosc".into(), kind: "bool".into(), desc: "true if return of spontaneous circulation is stated".into() },
                Field { key: "status".into(), kind: "string".into(), desc: "CPR in progress | ROSC | terminated | starting CPR | transporting with CPR | not arrest | unclear".into() },
                Field { key: "note".into(), kind: "string".into(), desc: "one line: rhythm, unit, location, or key clinical detail".into() },
            ],
            match_mode: "all".into(),
            conditions: vec![
                Clause { field: "arrest".into(), op: "==".into(), value: "true".into() },
                Clause { field: "cpr_in_progress".into(), op: "==".into(), value: "true".into() },
            ],
        ...
        AnalyzerRule {
            ...
            instructions: "From this fire/EMS radio transmission about a cardiac arrest, determine the arrest outcome. ROSC means return of spontaneous circulation: 'pulse back', 'ROSC', 'rhythm with pulse', 'palpable pulse', 'now breathing'. Terminated means resuscitation stopped without ROSC: 'pronounced', 'time of death', 'code terminated', 'resuscitation discontinued', 'per medical control'. If the arrest is still being worked and neither is stated, set outcome='ongoing'. Do not infer beyond what is stated.".into(),
            fields: vec![
                Field { key: "rosc".into(), kind: "bool".into(), desc: "true if return of spontaneous circulation is stated".into() },
                Field { key: "terminated".into(), kind: "bool".into(), desc: "true if resuscitation was stopped without ROSC".into() },
                Field { key: "outcome".into(), kind: "string".into(), desc: "ROSC | terminated | ongoing | unclear".into() },
                Field { key: "note".into(), kind: "string".into(), desc: "one line: time, rhythm, or key detail".into() },
            ],
            match_mode: "any".into(),
            conditions: vec![
                Clause { field: "rosc".into(), op: "==".into(), value: "true".into() },
                Clause { field: "terminated".into(), op: "==".into(), value: "true".into() },
            ],
