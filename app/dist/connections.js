...
(() => {
  ...
  async function renderFound() {
    ...
    $("cxFound").innerHTML = loose.length ? `<div class="k" style="margin-top:8px">Already used by your rules — name them</div>` + loose.map(([t, who]) => `<div class="row"><span class="grow"><b>${esc(describe(t))}</b> <span class="mono faint">${esc(t)}</span><br><small class="faint">${esc(who.filter((w) => w !== "default").join(", ") || "the default")}</small></span><button class="btn ghost sm" data-cxname-found="${esc(t)}">Name it</button></div>`).join("") : "";
